...
public interface BookRepository extends PagingAndSortingRepository<Book, Integer> {
    ...
    @Query("Select distinct b from Book b left join b.authors a " +
            "where b.idbook in" +
            "(select b.idbook from Book b left join b.authors a where b.bookName like CONCAT('%',?1,'%') or a.authorName like CONCAT('%',?2,'%')) " +
            "group by b.idbook having count(a) = ?3")
    List<Book> findByBookNameOrAuthorNameContainingAndAuthorCountIs(String text, String textAgain, long count, Pageable pageable);

    // (+) Year + Name + Count
    @Query("Select distinct b from Book b left join b.authors a " +
            "where b.bookYearWriting = ?1 and b.idbook in" +
            "(select b.idbook from Book b left join b.authors a where b.bookName like CONCAT('%',?2,'%') or a.authorName like CONCAT('%',?3,'%')) " +
            "group by b.idbook having count(a) = ?4")
